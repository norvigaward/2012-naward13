 ...
 * corpus.
 */
public class PairGeneratorMapper extends
	...
	public void map(Text key, ArcRecord record, Context context) throws IOException {
		...
		try {
			if(record.getHttpStatusCode() == 200 && record.getContentType().toLowerCase().equals("text/html")) {
				...
				Map<Pair, int[]> pairs = new HashMap<Pair, int[]>();
				...
				for(String sentence : sentences) {
					...
					if(isEnglish(words)) {					
						...
						// Create pairs
						if(interesting.size() <= MAX_SENTENCE_LENGTH) {	// Avoid creating too many pairs, only process short sentences
							// Create pairs
							for (int i = 0; i < interesting.size(); i++) {
								for (int j = i + 1; j < interesting.size(); j++) {
									String a = interesting.get(i);
									String b = interesting.get(j);
									
									if (!a.equals(b)) {		// Remove this to get word counts as (a,a) pairs
										Pair p = new Pair(a, b);
										
										int[] counts = pairs.get(p);
										if(counts == null) {
											counts = new int[MAX_SENTENCE_LENGTH * 2];
											pairs.put(p, counts);
										}
										...
										if(a.compareTo(b) > 0) {
											...
											counts[j - 1 + MAX_SENTENCE_LENGTH]++;
										}
									}
								}
							}
				...
				for (Pair pair : pairs.keySet()) {
					...
					for(int count : pairs.get(pair)) {
						out += count;
						out += " ";
					}
					
					context.write(new Text(pair.a + " " + pair.b), new Text(out)); // context.write(pair.a, pair.b)
				}
			}
		} catch (Throwable ex) {
