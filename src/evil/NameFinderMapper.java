 ...
 * corpus.
 */
public class NameFinderMapper extends
	...
	public void map(Text key, ArcRecord record, Context context) throws IOException {
		...
		try {
			...
			for(String sentence : sentences) {
				...
				// Remove markup
				sentence = removeTextMarkup(sentence);
				...
				if(isEnglish(words)) {					
					...
					for(String word : interesting) {
						if(first) {	// Ignore first word
							first = false;
						} else {
							if(word.toLowerCase().equals(word)) {
								context.write(new Text(word.toLowerCase()), new Text("0 1"));
							} else {
								context.write(new Text(word.toLowerCase()), new Text("1 0"));
							}
						}
					}
